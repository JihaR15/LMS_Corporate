...
export default function QuizClient({ moduleData, token }: { moduleData: ModuleData; token: string }) {
  ...
  const fetchQuestions = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/questions/module/${moduleData.id}`, {
  ...
  const confirmSingleDelete = () => {
    if (!deleteId) return;
    executeDelete(`${process.env.NEXT_PUBLIC_API_URL}/questions/${deleteId}`, "DELETE");
  };

  const confirmBulkDelete = () => {
    if (selectedIds.length === 0) return;
    executeDelete(`${process.env.NEXT_PUBLIC_API_URL}/questions/bulk-delete`, "POST", { ids: selectedIds });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!questionText || correctIndex === "" || answerOptions.some(ans => ans.trim() === "")) {
  ...
  // --- LOGIKA IMPORT EXCEL ---
  const handleDownloadTemplate = () => window.open("/template-soal.xlsx", "_blank"); 
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await res.json();
      if (res.ok) {
        setImportSuccessMessage(result.message || "Berhasil mengimpor soal dari Excel!");
        fetchQuestions(); 
      } else {
    ...
    <div className="max-w-7xl mx-auto pb-32">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div className="space-y-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Manajemen Soal Kuis</h2>
            <p className="text-sm text-slate-500 max-w-2xl mt-1">
              Modul: <strong className="text-slate-700">{moduleData.title}</strong>
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="bg-emerald-50 px-4 py-2 rounded-xl flex items-center gap-2 border border-emerald-100">
            <span className="material-symbols-outlined text-emerald-600 text-sm">quiz</span>
            <span className="text-xs font-bold text-emerald-800">Total: {questions.length} Soal</span>
          </div>
      ...
      <div className="flex flex-col gap-5 mt-5">
        {isLoading ? (
           <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-xs font-medium text-slate-500">Memuat bank soal...</p>
           </div>
        ) : questions.length > 0 ? (
          questions.map((q, index) => (
           ...
           <div className="text-center py-16 bg-white rounded-2xl border border-slate-200 shadow-sm">
             ...
             <p className="text-slate-500 text-sm max-w-sm mx-auto">
               Anda belum menambahkan soal evaluasi untuk modul ini. Klik tombol <strong>Tambah Soal</strong> atau <strong>Import dari Excel</strong> untuk memulai.
             </p>
           </div>
        )}
      </div>
          ...
          <div className="bg-white dark:bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50 dark:bg-slate-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800 dark:text-slate-800">Panduan Import Excel</h2>
                  <p className="text-xs text-slate-500 dark:text-slate-500">Unggah bank soal sekaligus menggunakan file Excel (.xlsx)</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto">
              <p className="text-sm text-slate-600 dark:text-slate-600 mb-4 leading-relaxed">
                Untuk mencegah error sistem, pastikan file Excel yang akan Anda unggah memiliki <strong>Header Baris Pertama</strong> yang persis (peka huruf besar/kecil) seperti format di bawah ini:
              </p>
              ...
              <div className="overflow-x-auto border border-slate-200 rounded-xl mb-6">
                <table className="w-full text-left text-sm whitespace-nowrap">
                  <thead className="bg-slate-100 dark:bg-slate-100 text-slate-700 dark:text-slate-700">
                    <tr>
                      <th className="px-4 py-3 font-bold border-b border-slate-200">Pertanyaan</th>
                      <th className="px-4 py-3 font-bold border-b border-slate-200">Opsi A</th>
                      <th className="px-4 py-3 font-bold border-b border-slate-200">Opsi B</th>
                      <th className="px-4 py-3 font-bold border-b border-slate-200">Opsi C</th>
                      <th className="px-4 py-3 font-bold border-b border-slate-200">Opsi D</th>
                      <th className="px-4 py-3 font-bold border-b border-slate-200">Jawaban Benar</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    <tr>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-600">Suhu pasteurisasi...</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-600">72° C</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-600">60° C</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-600">80° C</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-600">100° C</td>
                      <td className="px-4 py-3 text-emerald-600 font-bold bg-emerald-50 text-center">A</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl flex gap-3 text-amber-800 text-xs leading-relaxed">
                ...
                <p>
                  Kolom <strong>"Jawaban Benar"</strong> hanya boleh diisi dengan 1 huruf kapital saja: <strong>A, B, C, atau D</strong>. Jangan mencampur dengan teks lain. Anda dapat mengunduh format kosong di bawah ini agar lebih aman.
                </p>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-white w-full max-w-3xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-emerald-600 p-6 sm:p-8 text-white flex justify-between items-center shrink-0">
              <div>
                ...
                <p className="opacity-80 text-sm mt-1">Isi pertanyaan dan kunci jawaban dengan benar.</p>
              </div>
              <button 
            ...
            <div className="p-6 sm:p-8 overflow-y-auto">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-xs font-bold text-slate-600 dark:text-slate-600 mb-2 uppercase tracking-wide">Teks Pertanyaan</label>
                  <textarea 
                    value={questionText}
                    onChange={(e) => setQuestionText(e.target.value)}
                    required
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm focus:ring-emerald-500 focus:border-emerald-500 min-h-[100px] outline-none transition-all text-slate-800 dark:text-slate-800" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {['A', 'B', 'C', 'D'].map((letter, idx) => (
                    <div key={`input-${letter}`}>
                      <label className="block text-xs font-bold text-slate-600 dark:text-slate-600 mb-2 uppercase tracking-wide">Pilihan {letter}</label>
                      <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl p-1.5 focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-500 transition-all">
                ...
                <div className="pt-2">
                  <label className="block text-xs font-bold text-slate-600 dark:text-slate-600 mb-2 uppercase tracking-wide">Pilih Kunci Jawaban Benar</label>
                  <div className="relative">
                    <select 
                      value={correctIndex}
                      onChange={(e) => setCorrectIndex(Number(e.target.value))}
                      required
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-4 pr-10 appearance-none text-sm font-bold text-slate-700 dark:text-slate-700 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all cursor-pointer"
                    >
                      <option value="" disabled>Pilih salah satu (A, B, C, atau D)</option>
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-50 border-t border-slate-200 flex justify-end gap-3 shrink-0">
              ...
              <button 
                ...
                className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white px-8 py-2.5 rounded-xl font-semibold shadow-sm transition-all active:scale-95 text-sm"
              >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all">
          <div className="bg-white dark:bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden transform scale-100 transition-transform">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-800 mb-2">Hapus Soal?</h3>
              <p className="text-sm text-slate-500 dark:text-slate-500 mb-6">
                Soal ini beserta seluruh pilihan jawabannya akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.
              </p>
      ...
      {isBulkDeleteModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all">
          <div className="bg-white dark:bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden transform scale-100 transition-transform">
            <div className="p-6 text-center">
              <div className="w-16 h-16 bg-red-50 dark:bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4 border-4 border-red-100">
                <span className="material-symbols-outlined text-red-500 text-[32px]">delete_sweep</span>
              </div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-800 mb-2">Hapus {selectedIds.length} Soal Sekaligus?</h3>
              <p className="text-sm text-slate-500 dark:text-slate-500 mb-6">
                Seluruh soal yang dipilih beserta jawabannya akan dihapus secara permanen. Tindakan ini tidak dapat dibatalkan.
              </p>
              <div className="flex gap-3 justify-center">
                <button
      ...
      {importSuccessMessage && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-all">
          <div className="bg-white dark:bg-white w-full max-w-sm rounded-2xl shadow-xl overflow-hidden transform scale-100 transition-transform text-center p-6">
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-800 mb-2">Import Berhasil!</h3>
