...
export default async function OperatorDashboardPage() {
  ...
  // LOGIKA BARU: Modul dianggap TUNTAS (100%) HANYA JIKA ujiannya sudah lulus
  const completedModulesCount = modules.filter(
    (m: any) => m.is_test_passed === true || m.is_test_passed === 1,
  ).length;
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <section className="relative w-full h-65 flex items-center overflow-hidden bg-slate-900">
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-900 via-emerald-800 to-slate-900 z-10 opacity-90"></div>
        <div className="container mx-auto px-8 relative z-20 max-w-7xl">
          <div className="max-w-4xl">
            ...
            <h1 className="font-bold text-white text-3xl md:text-4xl leading-tight mb-3 break-words">
              Mari lanjutkan pembelajaranmu, {user?.fullname || "Operator"}!
            </h1>
            <p className="text-emerald-100/80 text-sm max-w-xl">
              Selesaikan modul harian Anda untuk memastikan standar kualitas dan
              keamanan produksi tetap terjaga di level tertinggi.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="container mx-auto px-8 max-w-7xl">
        <div className="flex justify-between items-end mb-8">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Selesaikan kurikulum wajib kuartal ini untuk mempertahankan
        ...
        <ModuleGridClient modules={modules} />

        {/* Bento Style Achievement & Support Section */}
          ...
          <div className="lg:col-span-2 bg-slate-100 rounded-2xl p-6 md:p-8 border border-slate-200 flex flex-col sm:flex-row gap-6 items-center">
            ...
            <div className="flex-1 w-full">
              ...
              <p className="text-slate-500 text-sm mb-4">
                Anda telah menuntaskan {completedModulesCount} dari{" "}
                {totalModules} seluruh kurikulum wajib posisi Anda. Pertahankan
                performa kerja Anda!
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <div className="bg-white p-3 rounded-xl text-center shadow-sm">
                  <span className="block text-xl font-bold text-emerald-600">
                    {modules.reduce(
                      (acc: number, cur: any) => acc + cur.completed_materials,
                      0,
                    )}
                  </span>
                  <span className="text-[9px] uppercase font-bold text-slate-400">
